import type { Geofence } from 'src/utils/geofence';
...
const GeofenceToolbar: React.FC<GeofenceToolbarProps> = ({
  ...
  const [wasExpandedBeforeDrawing, setWasExpandedBeforeDrawing] = useState(false);
  ...
  return (
    ...
    >
              ...
              geofences.map((geofence) => (
                <Box
                  ...
                  sx={{
                    p: 1,
                    mb: 1,
                    backgroundColor: selectedGeofence?.id === geofence.id ? '#e3f2fd' : 'inherit',
                    border:
      ...
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="sm" fullWidth>
        <DialogTitle>{editingGeofence ? '编辑地理围栏' : '新建地理围栏'}</DialogTitle>
        <DialogContent>
          <Box sx={{ pt: 1 }}>
            <TextField
              fullWidth
