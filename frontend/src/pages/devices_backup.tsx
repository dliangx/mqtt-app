...
export default function DevicesPage() {
  ...
  const handleDeleteGroup = async (id: number) => {
    try {
      await apiService.deleteDeviceGroup(id);
      enqueueSnackbar('设备组删除成功', { variant: 'success' });
      fetchDeviceGroups();
    } catch (err) {
      console.error('Failed to delete device group:', err);
      enqueueSnackbar('删除设备组失败', { variant: 'error' });
    }
  };
  ...
  const handleAddDevice = async () => {
    if (newDevice.group_id === 0) {
      const { group_id, ...deviceWithoutGroup } = newDevice;
      try {
        await apiService.createDevice(deviceWithoutGroup);
      } catch (err) {
  ...
  const filteredDevices =
    activeTab === 0
      ...
      : devices.filter(
          (device) =>
            device.group_id === (activeTab > 0 ? deviceGroups[activeTab - 1]?.id : undefined)
        );
