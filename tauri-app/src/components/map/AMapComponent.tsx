...
const AMapComponent = React.forwardRef<any, AMapComponentProps>(
  ({ devices, onMarkerClick, onGeofenceViolation, height = "400px" }, ref) => {
    ...
    // 获取用户当前位置
    const getUserCurrentLocation = useCallback((): Promise<{
      lat: number;
      lng: number;
    }> => {
      ...
      geofences.forEach((geofence) => {
        ...
        if (geofence.type === "polygon" && geofence.coordinates.length >= 3) {
          polygon = new window.AMap.Polygon({
            ...
            strokeColor:
              geofence.strokeColor || defaultGeofenceStyle.strokeColor,
            strokeOpacity: 1,
            strokeWeight:
    ...
    return (
      <div style={{ position: "relative", height }}>
        ...
        {/* 导航信息面板 */}
        {navigationInfo.visible &&
          navigationInfo.device &&
            ...
            >
              ...
              >
                <h4 style={{ margin: 0, color: "#1976d2" }}>
                  导航到 {navigationInfo.device.name}
                </h4>
                <button
                  onClick={clearRoute}
                  style={{
              ...
              >
                <div>
                  距离: {(navigationInfo.routeInfo.distance / 1000).toFixed(1)}{" "}
                  km
                </div>
                <div>
                  时间: {Math.ceil(navigationInfo.routeInfo.time / 60)} 分钟
                </div>
